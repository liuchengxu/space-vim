...
* 默认安装
    ...
    更多内容请直接查看各 layer 下的 README, config.vim 与 packages.vim.
...
** Basic
    ...
    | ~Ctrl f~      | /Insert/, /Command/ | forward, move forward one character                                         |
    | ~Ctrl b~      | /Insert/, /Command/ | backward, move backward one character                                       |
    | ~v~           | /Visual/            | ~<Esc>~                                                                     |
...
** Others
    ...
    | ~SPC f f~   | Normal | file search, 使用 fzf 搜索当前用户目录下的文件 (terminal vim) |
    | ~SPC f ?~   | Normal | file search, 使用 fzf 搜索当前工作目录下的文件 (terminal vim) |
    | ~SPC f R~   | Normal | reload .vimrc, 重新加载 .vimrc (/f/ ile /R/ esource)              |
    | ~SPC f r~   | Normal | file recently, 最近使用的文件 |
    | ~SPC f t~   | Normal | file toggle, NERDTree toggle, 打开 NERDTrer |
