...
** 启动时间
   ...
   从 [[https://github.com/junegunn/vim-startuptime-benchmark][vim-startuptime-benchmark]] 可以看出, 即使使用很多插件仍然可以保持很快的启动 ( 200 ms 以内). 尽管加载的插件不尽相同, 但仍具有借鉴意义.
   ...
   虽然比起 spacemacs 几秒钟的启动时间, 几百毫秒已经很快. 但对于 vim, 启动迅速是硬性要求, 而且当超过 900 ms 时, 就会有明显的 lag. 后来在这里找到了解决方案:[[https://github.com/junegunn/vim-plug/wiki/faq#loading-plugins-manually][load-plugins-manually]] , 主要是 YouCompleteMe 与 ultisnips 两个插件.
...
** space-vim statusline
   ...
   *[buffer-编号][TOT buffer 总数] 文件完整路径 『 ale 或 fugitive 信息』 文件编码 | 编码类型 | 行号:列号-虚拟列号 位置信息*

   file:../img/statusline.png

   通过 *buffer-编号*, 可以使用 ~SPC 编号~ 切换到对应 buffer. 通过 *[buffer 总数]* 知道当前共有几个 buffer, 使用 ~Tab~ 进行一次切换.
* 后记
